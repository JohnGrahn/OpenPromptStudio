...
app = modal.App.lookup("prompt-studio-sandbox", create_if_missing=True)
...
"""
        ...
        try:
            ...
            if not sb_is_up:
                ...
                await sb.set_tags.aio(
                    {"project_id": str(project_id), "app": "prompt-studio"}
                )
                print(
    ...
    async def prepare_sandbox(cls, stack: Stack) -> Tuple["DevSandbox", str]:
        vol_id = f"openprompt-vol-{_unique_id()}"
        ...
        await sb.set_tags.aio({"app": "openprompt"})
        await sb.wait.aio()
        return sb, vol_id